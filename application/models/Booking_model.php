<?php
	class Booking_model extends CI_Model{
		...
		public function get_bookings_report($start_date=null,$end_date=null,$carid=null){
			if($carid!=0){
				$query=$this->db->query("SELECT *,(SELECT SUM(clientbookings.rental_fee_current) from clientbookings where clientbookings.carId=cars.Id AND (clientbookings.start_date BETWEEN '".$start_date."' AND '".$end_date."')) AS Income,(SELECT SUM(clientbookings.number_of_days) from clientbookings where clientbookings.carId=cars.Id AND (clientbookings.start_date BETWEEN '".$start_date."' AND '".$end_date."')) AS TotalDays,(SELECT COUNT(*) from clientbookings where clientbookings.carId=cars.Id AND (clientbookings.start_date BETWEEN '".$start_date."' AND '".$end_date."')) AS NumberOfBooking FROM cars inner join clientbookings on clientbookings.carId=cars.Id WHERE clientbookings.carId=".$carid." AND (clientbookings.start_date BETWEEN '".$start_date."' AND '".$end_date."') GROUP by cars.Id ORDER BY cars.car_description");
			}else{
				$query=$this->db->query("SELECT *,(SELECT SUM(clientbookings.rental_fee_current) from clientbookings where clientbookings.carId=cars.Id AND (clientbookings.start_date BETWEEN '".$start_date."' AND '".$end_date."')) AS Income,(SELECT SUM(clientbookings.number_of_days) from clientbookings where clientbookings.carId=cars.Id AND (clientbookings.start_date BETWEEN '".$start_date."' AND '".$end_date."')) AS TotalDays,(SELECT COUNT(*) from clientbookings where clientbookings.carId=cars.Id AND (clientbookings.start_date BETWEEN '".$start_date."' AND '".$end_date."')) AS NumberOfBooking FROM cars inner join clientbookings on clientbookings.carId=cars.Id WHERE (clientbookings.start_date BETWEEN '".$start_date."' AND '".$end_date."') GROUP by cars.Id ORDER BY cars.car_description");
			}
			return $query->result_array();
